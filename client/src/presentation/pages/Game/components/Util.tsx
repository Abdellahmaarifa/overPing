...
let Util = ({
  ...
  if (loading) return <h1>loading... util</h1>;
  else if (error) return <h1>Error!...util</h1>;
  else {
    ...
    setTimeout(() => {
      //if (playerOne.friend === false)
      {
        ...
        p1.matchPlyed = data?.findProfileByUserId?.gameStatus
          .totalMatches as number;
        p1.matchWon = data?.findProfileByUserId?.gameStatus
          .matchesWon as number;
        p1.bestWinStreak = data?.findProfileByUserId?.gameStatus
          .best_win_streak as number;
        p1.level = data?.findProfileByUserId?.xp as number;
        p1.tournentWon = data?.findProfileByUserId?.gameStatus
          .win_streak as number;
        p1.tournentPlayed = data?.findProfileByUserId?.gameStatus
      ...
      <div className="UtilContainer">
        ...
        <div className="Utils">
          <div className="UtilContainers">
            <div
              className={`util1 ${Util === 1 ? "hoverUtil1" : ""}`}
              onClick={() => {
                setUtil(1);
              }}
            >
              <div className="UtilImg U1"></div>
              <div className="UtilName">
                <p>Mouse</p>
              </div>
            </div>
            <div
              className={`util2 ${Util === 2 ? "hoverUtil2" : ""}`}
              onClick={() => {
                setUtil(2);
              }}
            >
