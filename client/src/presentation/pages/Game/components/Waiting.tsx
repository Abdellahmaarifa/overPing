import {
  useJoinMatchGameMutation,
  useMatchWaitingListSubscription,
  useRemovePlayerFromQueueMutation,
} from "gql/index";
...
let Waiting = ({
  ...
  document.addEventListener("DOMContentLoaded", function () {
    plyOneImg?.setAttribute("src", playerOne.userAvatar);
    plyTwoImg?.setAttribute("src", "../../../assets/game/question-mark.jpeg");
  });
  ...
  let hideRobotOpetiondiv = () => {
    if (PlayWithRobot) PlayWithRobot.style.display = "none";
  };
  ...
  useEffect(() => {
    if (subscriptionData && subscriptionData.matchWaitingList) {
      ...
      setSwitchValue(true);
    } else console.log("Subscription data is null:");
  }, [subscriptionData]);
  ...
  if (TheSwitch === true) {
    ...
    if (
      ...
      (subscriptionData &&
        subscriptionData.matchWaitingList.matchKey === "null")
    ) {
      //console.log("====================> : ", subscriptionData)
      if (
        subscriptionData === undefined ||
        ...
        setTimeout(() => {
          plyTwoImg?.setAttribute(
            "src",
            "../../../assets/game/question-mark.jpeg"
          );
          setTimeout(() => {
            if (PlayWithRobot) PlayWithRobot.style.display = "inline-block";
          }, 100);
        }, 1000);
      }
      ...
      if (
        subscriptionData.matchWaitingList.user1 &&
        subscriptionData.matchWaitingList.user1.id &&
        playerOne.userId ===
          parseInt(subscriptionData.matchWaitingList.user1.id)
      ) {
        ply2MatchWager = subscriptionData.matchWaitingList.user2?.bet as number;
        if (
          subscriptionData.matchWaitingList.user2 &&
          subscriptionData.matchWaitingList.user2.id
        )
  ...
  return (
    <div className="WaitingContainer">
      ...
      <div className="WaitingVs">
        <div className="WaitingFlashLight">
          <p>VS</p>
        </div>
      </div>
      <div className="WaitingCoins">{/* <p>1000 &#128176;</p> */}</div>
      <div className="WaitingPlayer1">
        <div className="WaitingPlyInside1">
          {/* <p className="winLose"> </p> */}
          <div className="WaitingAvatarR1Cover">
            <div className="WaitingAvatarR1">
              <img
                id="WaitingAvatarR2"
                src="../../../assets/game/question-mark.jpeg"
                alt="Avatar"
              ></img>
            </div>
          </div>
      ...
      <div className="playWithRobot" id="robotReq">
        <p>
          No player found online <br />
          if you want to play with Robot <br /> click Yes
        </p>
