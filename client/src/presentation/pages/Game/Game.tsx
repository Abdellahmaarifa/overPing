...
import RobotImage from "assets/game/robot.jpg";
import BadgeImage from "assets/game/badge-3.png";
let gameCapsule: GameContainer = new GameContainer();
let playerOne: UserInfo = new UserInfo(
  tabId,
  "",
  0,
  0,
...
let playerTwo: UserInfo | undefined = new UserInfo(
  tabId,
  "",
  0,
  0,
...
let robot: UserInfo = new UserInfo(
  tabId,
  "",
  playerOne.matchWager,
  playerOne.modePlaying,
  "Mr Robot <|o_o|>",
  RobotImage,
  BadgeImage,
  10,
...
function ParentComponent({
  playerOne: renamePlayerOne,
  playerTwo: renamePlayerTwo,
}: {
  playerOne: UserInfo;
  playerTwo: UserInfo | undefined;
}) {
  let [localGameResult, setGameResult] = useState<Result>(
    () => new Result(0, 0, 0, 0, 0, 0, 0, 0)
  );
  ...
  const updatePlayerTwo = (
    ...
    newUserId: number,
    isFriend: boolean,
    newPly2userId: number,
    newMatchType
  ) => {
    setPlayerTwoState({
      ...playerTwo,
      playWithRobot: newPlayWithRobot,
      tabId: newTabId,
      matchId: newMatchId,
      matchWager: newMatchWager,
      modePlaying: newModePlaying,
      userName: newUserName,
      userAvatar: newUserAvatar,
      userLogo: newUserLogo,
      matchWon: newMatchWon,
      bestWinStreak: newBestWinStreak,
      matchPlyed: newMatchPlayed,
      level: newLevel,
      tournentPlayed: newTournentPlayed,
      tournentWon: newTournentWon,
      playWithMouse: newPlayWithMouse,
      userId: newUserId,
      friend: isFriend,
      ply2userId: newPly2userId,
  ...
  if (playerOne.playWithMouse === 0)
    return (
      <Util
        playerOne={playerOne}
        playerTwo={playerTwo as UserInfo}
        updatePlayerOne={updatePlayerOne}
        updatePlayerTwo={updatePlayerTwo}
        updateUserInfoUtil={UpdateUserInfoUtil}
      />
    );
  else if (playerOne.modePlaying === 0)
    return (
      <Modes playerOne={playerOne} updateUserInfoMode={updateUserInfoMode} />
    );
  else if (
    playerOne.matchWager === 0 &&
    playerOne.matchType === MatchMode.ONLINE_RANDOM
  )
    return (
      <Deposit
        playerOne={playerOne}
        updateUserInfoWager={updateuserinfoWager}
      />
    );
    ...
    return (
      <>
        <Waiting
          playerOne={playerOne}
          playerTwo={playerTwo as UserInfo}
          updateRobotOpetion={updateRobotOpetion}
  ...
  if (
    playerOne.matchId.length > 0 &&
    playerOne.playWithRobot === false &&
    readyState &&
    playerOne.friend === false
  )
    return (
      <Ready
        playerOne={playerOne}
    ...
    return (
      <ReadyRobot
        ...
        robot={robot}
        updateReadyState={updateReadyState}
        updateUserInfoWagerAndMode={updateUserInfoWagerAndMode}
        updateuserinfoWager={updateuserinfoWager}
        updateMatchId={updateMatchId}
        updatePlayerTwo={updatePlayerTwo}
        updatePlayerOne={updatePlayerOne}
    ...
    if (isServerDown === true) return <ServerDown />;
    else
      return (
        <>
          <div id="gameHover">
            <Info
              playerOne={playerOne}
              playerTwo={playerTwo as UserInfo}
              localGameResult={localGameResult}
    ...
    return (
      <BetterLuck playerOne={playerOne} playerTwo={playerTwo as UserInfo} />
    );
  return null;
}
