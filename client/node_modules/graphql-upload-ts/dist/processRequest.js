 ...
 * ```
 */
async function processRequest(req, res, uploadOptions) {
    const { maxFieldSize = 1000000, // 1 MB
    maxFileSize = Infinity, maxFiles = Infinity, environment, // "lambda", "gcf", "azure"
     } = uploadOptions !== null && uploadOptions !== void 0 ? uploadOptions : {};
    if (environment === 'gcf') {
        // Google Cloud Functions compatibility.
        if (!req.rawBody)
            throw new HttpError_1.HttpError(400, 'GCF req.rawBody is missing. See docs: https://cloud.google.com/functions/docs/writing/http#multipart_data');
    }
    else if (environment === 'lambda') {
        // AWS Lambda compatibility
        if (!req.body)
            throw new HttpError_1.HttpError(400, 'AWS Lambda req.body is missing. See these screenshots how to set it up: https://github.com/myshenin/aws-lambda-multipart-parser/blob/98ed57e55cf66b2053cf6c27df37a9243a07826a/README.md');
    }
    else if (environment === 'azure') {
        // Azure Functions compatibility
        req = req.req || res;
        if (!req || !req.body)
            throw new HttpError_1.HttpError(400, 'Azure Function req.body is missing. See this page for more info: https://docs.microsoft.com/en-us/azure/azure-functions/functions-reference-node');
    }
    else {
        // Regular node.js environment where request is a ReadableStream instance.
        if (!req || !req.pipe || !req.unpipe || !req.once || !req.resume)
            throw new HttpError_1.HttpError(400, "The request doesn't look like a ReadableStream. Tip: use `environment` option to enable serverless functions support.");
    }
    const contentType = req && req.headers && req.headers['content-type'];
    if (typeof contentType !== 'string' || !contentType.includes('multipart/form-data;')) {
        throw new HttpError_1.HttpError(400, `Invalid content-type ${contentType}, should be multipart/form-data;`);
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        const exit = (message, status = 400) => {
            ...
            if (message instanceof Error) {
                ...
                exitError = new HttpError_1.HttpError(status, message);
            }
            reject(exitError);
        ...
        parser.on('field', (fieldName, value, { valueTruncated /*, nameTruncated, encoding, mimeType */ }) => {
            ...
            switch (fieldName) {
                ...
                case 'map': {
                    if (!operations)
                        return exit(`Disordered multipart fields; 'map' should follow 'operations' (${SPEC_URL}).`);
                    let parsedMap;
                    try {
                    ...
                    for (const [fieldName, paths] of mapEntries) {
                        ...
                        for (const [index, path] of paths.entries()) {
                            ...
                            try {
                                ...
                                return exit(`Invalid object path for the 'map' multipart field entry key '${fieldName}' array index '${index}' value '${path}' (${SPEC_URL}).`);
                            }
                        }
                    }
                    resolve(operations);
        ...
        parser.on('file', (fieldName, stream, { filename, encoding, mimeType: mimetype }) => {
            ...
            let fileError;
            stream.on('limit', () => {
                fileError = new HttpError_1.HttpError(413, `File truncated as it exceeds the ${maxFileSize} byte size limit.`);
                stream.unpipe();
            });
            ...
            const file = {
                ...
                createReadStream(...args) {
                    if (args && args.some(Boolean)) {
                        throw new Error('graphql-upload-ts does not support createReadStream() arguments. Use graphql-upload NPM module if you need this feature.');
                    }
                    const error = fileError || (released ? exitError : null);
                    if (error)
                        throw error;
                    if (returnedStreams.has(stream)) {
                        throw new Error("graphql-upload-ts does not allow calling createReadStream() multiple times. Please, consume the previously returned stream. Make sure you're not referencing same file twice in your query.");
                    }
                    else {
        ...
        parser.once('close', () => {
            ...
            req.resume();
            if (!operations && !map) {
                return exit(`graphql-upload-ts couldn't find any files or JSON. Looks like another middleware had processed this multipart request. Or maybe you are running in a cloud serverless function? Then see README.md.`, 500);
            }
            if (!operations)
            ...
            for (const upload of map.values())
                if (!upload.file)
                    upload.reject(new HttpError_1.HttpError(400, 'File missing in the request.'));
        });
